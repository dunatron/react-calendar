...
class PullEventFindaEvents extends BuildTask
{
    ...
    public function StoreEvents($collection)
    {
        foreach ($collection->events as $event) {
            ...
            $newEvent->Website = $event->url;
            $newEvent->Description = $event->description;
            // $newEvent->CalendarPageID = 1;
            $newEvent->Approved = 1;
            $newEvent->IsEventFindaEvent = 1;
            $newEvent->Venue = $event->location->name;
            // venue name
            $newEvent->LocationText = $event->location->summary;
            // Location Summary
            $newEvent->Lat = $event->point->lat;
            // Lon address
            $newEvent->Lon = $event->point->lng;
            // Event Date
            $newEvent->Date = $event->datetime_start;
            // Start Time
            $newEvent->Start = $event->datetime_start;
            // Finish Time
            $newEvent->Finish = $event->datetime_end;
            $newEvent->Free = $event->is_free;

            // Try store website if we have one. (just get the first site)
            if (!empty($event->web_sites->web_sites)) {
                $newEvent->Website = $event->web_sites->web_sites[0]->url;
            }
    ...
    public function getTagByName($tagName, $tagType)
    {
        ...
        switch ($tagType) {
            case "HappTag":
                $Tag = HappTag::get()->filter(array(
                    'Title' =>  $tagName
                ))->first();
                break;
            case "SecondaryTag":
                $Tag = SecondaryTag::get()->filter(array(
                    'Title' =>  $tagName
                ))->first();
        }
